...
## 
## knitr::purl(input=here::here("data", 'dataprep.Rmd'),
...
ab<- read_csv(here::here("data", "raw", "ab", "Caribou_Demographic_Vital_Rates_22July2021_forSerrouya.csv"))%>%
  mutate( Reporting_Pop=case_when( Reporting_Pop%in%"Narraway"~"Narraway AB", TRUE~Reporting_Pop), ##break out AB data
          Year_START=Year_END-1 ) ##switch reporting year to start year
...
##Tonquin
tonq <- read_csv(here::here("data", "raw", "Tonquin_Moeller2020.csv"))
...
treat.raw <- treat.raw%>%filter(Herd%in%herds)%>%
  mutate(end.year=case_when(end.year>year.cut~year.cut,
                            TRUE~end.year))
...
surv <- surv%>%
  mutate(id=case_when(str_starts(id,"CN") & herd%in%c("Klinse-Za","Chase","Wolverine")~str_sub(id,1,-2),
         TRUE~id))
...
surv<-surv%>%
  mutate(DateEntry.bio=case_when(!herd%in%c(spring.herds,fall.herds)~DateEntry-offset.bioyr,
                                 herd%in%c(spring.herds)~DateEntry-offset.bioyr_spring,
                                 herd%in%c(fall.herds)~DateEntry-offset.bioyr_fall),
         DateExit.bio=case_when(!herd%in%c(spring.herds,fall.herds)~DateExit-offset.bioyr,
...
surv.yr <- surv.day%>%
  ...
  summarise(enter.date=min(DateExit.bio),
...
for(i in 1:length(surv.herds)){
    ...
    surv.yr.est <- rbind(surv.yr.est, 
                         data.frame(herd=surv.herds[i],
...
##add in AB survival
surv.yr.est <- rbind(surv.yr.est,
                     data.frame(herd=ab$Reporting_Pop,
                            year=ab$Year_START,
...
surv.yr.est <- surv.yr.est%>%
  mutate(n=case_when(is.na(n)~15,TRUE~n), #fill in sample size for AB data, this is approximate, but doesn't matter much, rarely surv==1 or 0 w/ no error
         est=case_when(n<=2~NA_real_,
                       TRUE~est))%>%
  drop_na(est)
...
for(i in 1:1000){
  surv.boot.i<- surv.rep%>%
    ...
    dplyr::summarise(mean=mean(surv),
              n=max(n),
              .groups="keep")%>%
    dplyr::mutate(iter=i)
...
surv.yr.est <-surv.yr.est%>%
  ...
  mutate(se=case_when(est%in%c(1,0)~sd.boot ,
                      TRUE~se),
         est=case_when(est%in%c(0)~0.5 ,
                      TRUE~est))%>%
...
##denote seasons
sr <- sr%>%
mutate(season=case_when(`Official Survey Timing`%in%c("Winter","March","April","February")~"winter",
...
kz.sr <- tibble(herd="Klinse-Za",
                ...
                ad.total=NA,
                ad.id=NA,
                sratio=c(0.68,0.71,0.64,0.65,0.51,0.55,0.43,0.44),
                sratio.sd=0.05)
...
mutate(`Unclassified Life Stage and Sex (OTC)`=case_when(is.na(`Unclassified Life Stage and Sex (OTC)`)~0, ###Make the NA's in unclassified ==0
                                                           TRUE~`Unclassified Life Stage and Sex (OTC)`),
         `Unclassified Life Stage and Sex (OSC)`=case_when(is.na(`Unclassified Life Stage and Sex (OSC)`)~0,
                                                           TRUE~`Unclassified Life Stage and Sex (OSC)`),
         `Unclassified Life Stage and Sex (Min # Known Alive)`=case_when(is.na(`Unclassified Life Stage and Sex (Min # Known Alive)`)~0,
                                                                               TRUE~`Unclassified Life Stage and Sex (Min # Known Alive)`))%>%
  mutate(`N. Adults (OTC)`=`Survey Count (KMB = SO, Total count) OTC`-`N. Calves (OTC)`-`Unclassified Life Stage and Sex (OTC)`, ###get to adults only
  ...
  mutate(r.OTC=`N. Calves (OTC)`/`N. Adults (OTC)`,
  ...
  mutate(season=case_when(`Official Survey Timing`%in%c("Winter","March","April","February")~"winter",
                          ...
                          is.na(`Official Survey Timing`)~NA_character_,
                          TRUE~"needs class"))%>%
  select(herd, year=Year,season, r.OTC, `N. Calves (OTC)`,`N. Adults (OTC)`,
...
recruitment <- tibble(herd=recruitment$herd,
...
recruitment <- recruitment%>%
  ...
  mutate(calves=case_when(!is.na(recruitment_OSC)~calves_OSC,
                          !is.na(recruitment_MNKA)~calves_MNKA,
                          !is.na(recruitment_OTC)~calves_OTC),
         adults=case_when(!is.na(recruitment_OSC)~adults_OSC,
                          !is.na(recruitment_MNKA)~adults_MNKA,
                          !is.na(recruitment_OTC)~adults_OTC),
         recruitment=case_when(!is.na(recruitment_OSC)~recruitment_OSC,
                               !is.na(recruitment_MNKA)~recruitment_MNKA,
                                !is.na(recruitment_OTC)~recruitment_OTC),
         count.used=case_when(!is.na(recruitment_OSC)~"OSC",
...
recruitment%>%
  ...
  group_by(count.used)%>%
  summarise(mean=mean(recruitment),
            sd=sd(recruitment),
            max=max(recruitment),
            n=n())
...
##remove herd-years with <=2 aadults seen (means survival est could only be 0,0.5,or 1)
recruitment <- recruitment%>%
  mutate(adults=case_when(adults<=2~NA_real_,
                       TRUE~adults))%>%
  drop_na(adults)
...
recruitment <- recruitment%>%
      group_by(herd,year)%>%
  mutate(se=sqrt((recruitment*(1-recruitment))/sum(calves+adults)),
...
recruitment <-recruitment%>%
  mutate(year=case_when(season%in%c("spring","fall")~year+1,
                        TRUE~year))

##Prioritize surveys in a given year based on winter>fall>spring
recruitment <-recruitment%>%
  mutate(season_int=case_when(season%in%"winter"~1,
                              season%in%"fall"~2,
                              season%in%"spring"~3))%>%
  group_by(herd,year)%>%
...
otc.sr <- sr.summary%>%filter(type%in%"OTC")%>%pull(mean)
osc.sr <- sr.summary%>%filter(type%in%"OSC")%>%pull(mean)
mnka.sr <- sr.summary%>%filter(type%in%"MNKA")%>%pull(mean)

recruitment <- recruitment%>%
  left_join(sr%>%filter(type%in%c("OSC","MNKA"))%>%select(herd,year,season,sratio), by=c("herd","season","year"))%>%
  mutate(sratio=case_when(count.used%in%"OSC" & is.na(sratio)~osc.sr,
                       count.used%in%"MNKA" & is.na(sratio)~mnka.sr,
                       TRUE~sratio),
         est.adj=case_when(count.used%in%c("OSC","MNKA")~est*(otc.sr/sratio),
                       TRUE~est))
...
counts <- count.raw%>%
  filter(!NFG%in%c("Y"))%>%
  mutate(`N collars detected`=case_when(is.na(`Survey Count (KMB = SO, Total count) OTC`)~NA_real_, TRUE~`N collars detected`),
         `N collars available`=case_when(is.na(`Survey Count (KMB = SO, Total count) OTC`)~NA_real_, TRUE~`N collar available`))%>%
  mutate(`Survey Count (KMB = SO, Total count) OTC`=case_when(herd%in%c("Chase","Wolverine") & !is.na(`Estimate (MC, eg JHE)`)~(`Estimate (MC, eg JHE)`*(`N collars detected`/`N collar available`)),
         TRUE~`Survey Count (KMB = SO, Total count) OTC`))%>% ## back transform Chase and Wolverine
  select(herd, 
         year=Year,
  ...
  mutate(Sightability=CollarsSeen/CollarsAvailable,
...
herd.sight <- counts%>%
  group_by(herd)%>%
  summarise(est=sum(CollarsSeen,na.rm=TRUE)/sum(CollarsAvailable,na.rm=TRUE),
...
counts <- counts%>%
  ...
  mutate(Sightability.pooled=case_when(is.na(Sightability) | Sightability%in%0~ mean(Sightability, na.rm=TRUE),
                                TRUE~Sightability),
                  Est_CL=case_when(Sightability.pooled>0~SurveyCount/Sightability.pooled,
                          TRUE~NA_real_))%>% 
...
counts <- counts%>%
  mutate(count=case_when(Est_CL>=MinCount~SurveyCount,
                         Est_CL<MinCount~MinCount,
                         is.na(MinCount)~SurveyCount,
                         is.na(SurveyCount)~MinCount),
         Sightability=case_when(is.na(MinCount)~Sightability,
                          Est_CL>=MinCount~Sightability,
                         Est_CL<MinCount~0.99,
                         is.na(SurveyCount)~0.99),
         MinUsed=case_when(Est_CL<MinCount~1,
                           is.na(SurveyCount)~1,
                           TRUE~0))%>%
    mutate(sd=case_when(Sightability==0.99~ mean(sd,na.rm=TRUE),
                      TRUE~sd))
...
counts <-counts%>%
  ...
  mutate(sd=case_when(Sightability%in%c(1,0)~sd.boot ,
                      TRUE~sd))%>%
  ungroup%>%
  select(-av, -sd.boot,-n)
...
counts <- counts%>%
  ungroup()%>%
  mutate(Est_CL=case_when(is.na(Sightability) | Sightability%in%0~count/Sightability.pooled,
                          !is.na(Sightability)~count/Sightability),
         Est_CL.min=case_when(is.na(Sightability) | Sightability%in%0~count/(Sightability.pooled+mean(se,na.rm=TRUE)),
                          !is.na(Sightability)~count/(Sightability+sd)),
          Est_CL.max=case_when(is.na(Sightability) | Sightability%in%0~count/(Sightability.pooled-mean(se,na.rm=TRUE)),
                          !is.na(Sightability)~count/(Sightability-sd)))%>%
  drop_na(count)
...
counts <- counts%>%
  rbind(
    tibble(herd=c("A La Peche","Narraway AB","Redrock/Prairie Creek"),
...
counts <- counts%>%
  rbind(tonq%>%
          filter(Parameter%in%"Abundance")%>%
          group_by(Year)%>%
          summarise(SurveyCount=sum(Median)*0.99,
                    sd=sum(SD))%>%
          ungroup%>%
mutate(herd=c("Tonquin"),
  year=Year,
  timing="October",
  MinCount=NA,
  CollarsSeen=NA,
  ...
  sd=sd,
  Sightability.pooled=NA,
  Est_CL=SurveyCount/0.99,
  count=SurveyCount/0.99,
  MinUsed=0,
  Est_CL.min=(SurveyCount/0.99)-sd,
  Est_CL.max=(SurveyCount/0.99)+sd
  )%>%
...
counts%>%
  ...
  group_by(herd)%>%
  summarise(available=sum(CollarsAvailable),
...
treat.raw <- treat.raw%>%
  mutate(treatment=case_when(treatment%in%"sterilization"~"sterilize wolves",
...
trt_raw <- treat.raw %>%
  rename(herd=Herd)%>%
  filter(herd%in%c(counts$herd,surv$herd, recruitment$herd),
         !treatment%in%"all")%>%
...
trt_raw <- trt_raw%>%
  ...
  ungroup()%>%
  mutate(start.year=case_when(!treatment%in%"none"~start.year+1,
                              TRUE~start.year),
         end.year=case_when(!treatment%in%"none"~end.year+1,
                              TRUE~end.year))

# Herds and herd number
herds <- unique(trt_raw$herd)
hn <- data.frame(herd = herds,
...
ntrts <- length(u_trts)
possible_trts <- data.frame(year = rep(yrs, times = ntrts+1),
...
for(i in 1:nherd){
	...
	for(j in 1:length(u_trts_h)){
		trt_h <- data.frame(herd = rep(herds[i], times = nyr),
			year = yrs,
...
bp <- tibble(herd=unique(trt_long$herd),
...
sight_group <-treat.raw%>%
  mutate(sight_grp=draft.sightability.cluster%>%as.factor%>%as.numeric)%>%
  select(herd=Herd,sight_grp)%>%
  distinct()
...
## 
## ##export raw data
## write_csv(trt_raw%>%filter(herd%in%herds[i])%>%select(herd,treatment,start.year,end.year), here::here("data","herd_checks",herds[i],"raw_data","treatments_raw.csv"))
## write_csv(count.raw%>%filter(herd%in%herds[i]), here::here("data","herd_checks",herds[i],"raw_data","counts_raw.csv"))
## write_csv(recruitment%>%select(herd,year,calves,adults)%>%filter(herd%in%herds[i]), here::here("data","herd_checks",herds[i],"raw_data","recruitment_raw.csv"))
...
## write_csv(counts%>%select(herd,year,count,Sightability,sd,MinUsed, Est_CL,sexratio,sexratio.sd)%>%filter(herd%in%herds[i]), here::here("data","herd_checks",herds[i],"cleaned_data","counts.csv"))
## write_csv(recruitment%>%select(herd,year,est,sd)%>%filter(herd%in%herds[i]), here::here("data","herd_checks",herds[i],"cleaned_data","recruitment.csv"))
## write_csv(surv.yr.est%>%select(herd,year,est,sd)%>%filter(herd%in%herds[i]), here::here("data","herd_checks",herds[i],"cleaned_data","survival.csv"))
## 
## ##plots
## recruitment%>%select(herd,year,est,sd)%>%filter(herd%in%herds[i])%>%
...
## 
## 
## surv.yr.est%>%select(herd,year,est,sd)%>%filter(herd%in%herds[i])%>%
## ggplot(aes(x=year, y=est))+
##   geom_pointrange(aes(ymin=est-sd, ymax=est+sd), shape=21)+
##   theme_ipsum()+
##   theme(legend.position = "none")+
...
## 
## 
## recruitment%>%select(herd,year,calves,adults)%>%filter(herd%in%herds[i])%>%pivot_longer(calves:adults)%>%
...
##   select(`Animal ID (RS, CL)`,`Date entry`,`Date exit`,Outcome)%>%
##   group_by(`Animal ID (RS, CL)`)%>%
##   mutate(`Date exit`=ymd(`Date exit`),
...
## 
## 
## ggplot(counts%>%filter(herd%in%herds[i])%>%
##          mutate(year=as.integer(year))%>%
##          pivot_longer(SurveyCount:Est_CL)%>%
##          filter(name%in%c("SurveyCount","MinCount","Estimate","Est_CL")),
##        aes(x=year, y=value, color=name))+
...
a <-counts%>%
  mutate(season=case_when(timing%in%c("Winter","March","April","February")~"winter",
                          timing%in%c("August","September","October","November")~"fall",
                          timing%in%c("June","July")~"spring"))%>%
  select(herd, year, season,est=Est_CL)%>%mutate(param="count")%>%
