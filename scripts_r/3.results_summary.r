...
demog <- out %>%
  ...
  left_join(yr_df %>% rename(j = yr_idx)) %>% # would probably prefer the by syntax and not allowing natural join here, explicit is good
  left_join(hd %>% select(herd, i = herd_num)) %>% # same as above
  left_join(treatment.combos) %>% # specify by columns
  mutate(trt = case_when(is.na(trt) ~ "Reference", TRUE ~ trt)) %>% # why not just use tidyr::replace_na
...
sim <- sims_tmp %>%
  ...
  dplyr::left_join(treatment.combos, by = c("herd", "year")) |>
  dplyr::mutate(
...
demog.draws <- demog.draws %>%
  group_by(herd, .draw) %>%
  arrange(yrs) %>% # Note that we have the yrs v year problem here as well
  slice(-1) %>%
  ungroup()
...
eff.draws <- demog.draws %>%
  ...
  summarise(across(lambda:R, ~ mean(.x))) %>% # Don't see the exp portion of the geo mean
